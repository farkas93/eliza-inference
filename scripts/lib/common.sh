...
load_env() {
  ...
  SGLANG_VENV="${SGLANG_VENV:-$ELIZA_VENV_DIR/sglang}"
  ...
  export MODEL_HOME HF_HOME LLAMA_CACHE LOG_DIR RUNTIME_STATE_FILE ELIZA_VENV_DIR BASE_VENV STT_VENV TTS_VENV VLLM_VENV SGLANG_VENV VOCODE_VENV DEFAULT_HOST ELIZA_MEDIUM_PORT ELIZA_SMALL_PORT STT_PORT TTS_PORT VOCODE_BRIDGE_PORT VLLM_BIN LLAMA_SERVER_BIN PIPER_BIN
}
...
venv_python() {
  ...
  case "$service" in
    ...
    sglang|eliza-medium-sglang) echo "$SGLANG_VENV/bin/python" ;;
...
Examples:
  ./scripts/$command_name eliza-medium --profile medium/qwen3.6-35b-a3b-q4-llamacpp-256k
...
resolve_profile_path() {
  ...
  # Load alias mappings from external file for maintainability
  alias_file="$ROOT_DIR/configs/profiles/aliases.sh"
...
parse_service_profile() {
  ...
  if [[ -z "$PROFILE" ]]; then
    case "$SERVICE" in
      eliza-medium) PROFILE="medium/deepseek-v4-flash-ds4-256k" ;;
